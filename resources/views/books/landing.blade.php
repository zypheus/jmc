...
<body class="opac-body">
    @if($searchActive)
        <header class="opac-search-header" role="banner">
            <div class="opac-search-header-inner">
                <a href="{{ route('library.landing') }}" class="opac-search-brand text-decoration-none">
                ...
                <form method="GET" action="{{ route('library.landing') }}" class="opac-search-header-form" aria-label="Search">
                    ...
                    <div class="opac-search-header-meta">
                        ...
                        @endif
                        <a href="{{ route('library.landing', ['view' => ($viewMode ?? 'books')]) }}" class="opac-search-clear-link">Clear search</a>
                    </div>
                </form>
            </div>
        </header>
    @else
        <header class="opac-public-header opac-header-bar">
            <div class="logo opac-logo-wrap">
                <a href="{{ route('library.landing') }}" class="text-decoration-none text-dark d-inline-flex align-items-center">
                    <img src="{{ asset('images/pantasLogo.png') }}" alt="Library Logo">
                </a>
            </div>
            <nav class="opac-top-nav" aria-label="Quick links">
                <a href="{{ url('/') }}" class="opac-nav-link">Home</a>
                <a href="{{ route('library.kiosk.scan') }}" class="opac-nav-link">Student lookup</a>
                <a href="{{ route('library.landing') }}" class="opac-nav-link fw-semibold">Catalog</a>
            </nav>
    ...
    @unless($searchActive)
        <section class="opac-hero-search" aria-labelledby="opac-search-heading">
            <div class="opac-hero-search-inner">
                ...
                <form method="GET" action="{{ route('library.landing') }}" class="opac-search-form opac-hero-search-form">
        ...
        <section class="opac-links-block" aria-labelledby="opac-links-heading">
            <div class="opac-links-inner">
                ...
                <ul class="opac-links-list">
                    ...
                    <li>
                        <a class="opac-link-card" href="{{ url('/') }}">
        ...
        <aside class="opac-facets" aria-label="Filters">
            <div class="opac-filters-panel">
                <section class="opac-filter-group">
                    ...
                    <a class="opac-facet-link" href="{{ route('library.landing', array_merge(request()->except('page'), ['view' => (($viewMode ?? 'books') === 'ebooks' ? 'books' : 'ebooks')])) }}">
                ...
                <section class="opac-filter-group">
                    ...
                    <form method="GET" action="{{ route('library.landing') }}" class="opac-facet-form">
        ...
        <main class="opac-results-panel">
            ...
            @if((($viewMode ?? 'books') === 'ebooks' ? ($ebooks?->total() ?? 0) : $books->total()) === 0)
                <div class="opac-results-empty">
                    ...
                    <a href="{{ route('library.landing') }}" class="btn btn-outline-primary btn-sm mt-2">Back to catalog home</a>
                </div>
            @endif
    ...
    <script>
        window.CHECKOUT_URL = "{{ route('library.checkout.process') }}";
        window.RESERVE_URL = "{{ route('library.opac.reserve') }}";
