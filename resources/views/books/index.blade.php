...
<div class="books-index-layout">
    ...
    <aside class="books-index-sidebar card p-3">
        ...
        <a href="{{ route('library.book.index', ['show_all' => 1]) }}"
           class="btn btn-primary w-100 mb-3 {{ $showAll && !request('search') && !request('program') && !request('year1') && !$statusFilter ? 'active' : '' }}">
            Show all books
        </a>

        <form action="{{ route('library.book.index') }}" method="GET" class="books-sidebar-form">
            @if($statusFilter)
            ...
            @if($hasActiveQuery)
                <a href="{{ route('library.book.index') }}" class="btn btn-outline-secondary w-100 btn-sm">Clear &amp; start over</a>
            @endif
        </form>
        ...
        <nav class="books-sidebar-nav">
            <a href="{{ route('library.book.index', array_merge(request()->except('status', 'page'), ['status' => 'Available'])) }}"
               class="btn btn-available w-100 {{ $statusFilter === 'Available' ? 'active' : '' }}">Available</a>
            <a href="{{ route('library.book.index', array_merge(request()->except('status', 'page'), ['status' => 'Borrowed'])) }}"
        ...
        <nav class="books-sidebar-nav">
            <a href="{{ route('library.book.create') }}" class="btn btn-addbook w-100">Cataloging</a>
            <a href="{{ route('library.ebooks.index') }}" class="btn btn-e-book w-100">View E-Resources</a>
            <a href="{{ route('library.books.archived') }}" class="btn btn-secondary w-100">Archived</a>
            <a href="{{ route('library.books.trash') }}" class="btn btn-outline-danger w-100">Trash</a>
        </nav>

        <hr class="my-3">

        <h6 class="books-sidebar-heading">Import / export</h6>
        <form action="{{ route('library.books.import') }}" method="POST" enctype="multipart/form-data" class="books-sidebar-form">
            @csrf
            ...
            @if($hasActiveQuery)
                <a href="{{ route('library.export.books', request()->query()) }}" class="btn btn-export w-100">Export results</a>
            @else
    ...
    <main class="books-index-main">
                                        ...
                                        <td class="text-end">
                                            <div class="dropdown1">
                                                ...
                                                <div class="dropdown1-content">
                                                    <a href="{{ route('library.book.show', $copy->id) }}" class="dropdown-item1">View</a>
                                                    <a href="{{ route('library.book.edit', $copy->id) }}" class="dropdown-item2">Edit</a>
                                                    <form action="{{ route('library.books.archive', $copy->id) }}" method="POST">
                                                        @csrf
                                                ...
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content rounded-3 shadow-lg">
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            <form action="{{ route('library.books.destroy', $copy->id) }}" method="POST">
                                                                @csrf
                                                                @method('DELETE')
                                    ...
                                    @else
                                        ...
                                        <td class="text-end">
                                            <div class="dropdown1">
                                                <button type="button" class="dropdown1-button">Actions</button>
                                                <div class="dropdown1-content">
                                                    <a href="{{ route('library.books.copies.staff', [
        ...
        @else
            ...
            <div class="card books-index-welcome p-5 text-center">
                ...
                <a href="{{ route('library.book.index', ['show_all' => 1]) }}" class="btn btn-primary btn-lg">
                    Show all books
                </a>
            </div>
