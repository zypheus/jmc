...
<div class="container">
    ...
    <div class="upload-card">
        ...
        <form action="{{ route('library.files.upload') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="row g-3">
                <div class="col-md-6">
            ...
            <div class="d-flex justify-content-between flex-wrap gap-2 mt-3">
                ...
                <a href="{{ route('library.book.index') }}" class="btn btn-secondary">
        ...
        <aside class="repo-sidebar">
            <div class="card shadow-sm">
                ...
                <div class="list-group list-group-flush">
                    <a href="{{ route('library.files.index') }}"
                    ...
                    @forelse($folderCounts as $folderKey => $count)
                        <a href="{{ route('library.files.index', ['folder' => $folderKey]) }}"
                    ...
                    <div class="card file-card mb-3">
                        <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-2">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                <a href="{{ route('library.files.view', $file->id) }}" target="_blank" class="btn btn-outline-info btn-sm">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="{{ route('library.files.download', $file->id) }}" class="btn btn-outline-success btn-sm" download>
                                    <i class="bi bi-download"></i>
                                </a>
                                <form action="{{ route('library.files.delete', $file->id) }}" method="POST" onsubmit="return confirm('Delete this file?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @forelse($filesByFolder as $folderKey => $folderFiles)
                    <div class="repo-folder-block">
                        <p class="repo-folder-heading mb-0 d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ route('library.files.index', ['folder' => $folderKey]) }}" class="btn btn-sm btn-light">Open folder</a>
                        </p>
                            ...
                            @foreach($folderFiles as $file)
                                <li class="list-group-item d-flex flex-wrap justify-content-between align-items-center gap-2">
                                    ...
                                    <div class="d-flex flex-shrink-0 gap-1">
                                        <a href="{{ route('library.files.view', $file->id) }}" target="_blank" class="btn btn-outline-info btn-sm py-0"><i class="bi bi-eye"></i></a>
                                        <a href="{{ route('library.files.download', $file->id) }}" class="btn btn-outline-success btn-sm py-0"><i class="bi bi-download"></i></a>
                                        <form action="{{ route('library.files.delete', $file->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this file?')">
                                            @csrf
                                            @method('DELETE')
