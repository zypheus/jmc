...
@section('content')
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        ...
        <div class="d-flex gap-2">
            <a href="{{ route('library.admin.catalog_frameworks.index') }}" class="btn btn-outline-secondary btn-sm">MARC frameworks</a>
            ...
            <div class="col-lg-4" id="field-{{ $anchor }}">
                <div class="card h-100 shadow-sm {{ request('field') === $anchor ? 'border-primary' : '' }}">
                    ...
                    <div class="card-body">
                            ...
                            <ul class="list-group list-group-flush mb-3">
                                @foreach ($section['saved'] as $opt)
                                    <li class="list-group-item d-flex justify-content-between align-items-center px-0">
                                        <span>{{ $opt }}</span>
                                        <form method="POST" action="{{ route('library.admin.catalog_select_options.destroy') }}"
                        ...
                        <form method="POST" action="{{ route('library.admin.catalog_select_options.store') }}" class="row g-2 align-items-end">
                            @csrf
                            <input type="hidden" name="tag" value="{{ $def['tag'] }}">
                            <input type="hidden" name="subfield" value="{{ $sub }}">
                            <div class="col">
