...
class LibraryHoldingsReportBuilder
{
     ...
     * }
     */
    public function buildForProgram(LibraryProgram $program, ?int $recentYears = null): array
    {
     ...
     * }
     */
    public function buildReport2(LibraryProgram $program): array
    {
     ...
     */
    protected function booksForProgram(LibraryProgram $program, bool $requireCourse = false): Collection
    {
     ...
     * @return list<int>
     */
    protected function programIdsForHoldings(LibraryProgram $program): array
    {
        ...
        $ids = $ids->merge(
            LibraryProgram::where('program_code', $code)->pluck('id')
        );

        $baseCode = preg_replace('/[\-_\s].*$/', '', $code) ?: $code;
        if (strlen($baseCode) >= 2) {
            $ids = $ids->merge(
                LibraryProgram::query()
    ...
    {
        $groups = $books->groupBy(function (LibraryBook $book) {
    ...
    protected function buildReport2SummaryLines(Collection $books): array
    {
        ...
        foreach (config('reports.report2_summary_order', []) as $classification) {
            $classified = $books->filter(
                fn (LibraryBook $book) => $this->report2ClassificationLabel($book->curriculum) === $classification
            );

            $titleGroups = $classified->groupBy(
                fn (LibraryBook $book) => mb_strtolower(trim((string) $book->title_statement))
            );
            ...
            foreach ($titleGroups as $copies) {
                $hasPrinted = $copies->contains(fn (LibraryBook $book) => $this->collectionTypeFor($book) === 'Printed');
                $hasElectronic = $copies->contains(fn (LibraryBook $book) => $this->collectionTypeFor($book) === 'Electronic');
            ...
            $printedVolumes = $classified
                ->filter(fn (LibraryBook $book) => $this->collectionTypeFor($book) === 'Printed')
                ->count();
            $electronicVolumes = $classified
                ->filter(fn (LibraryBook $book) => $this->collectionTypeFor($book) === 'Electronic')
                ->count();
     ...
     * @return list<string>
     */
    protected function prospectusCourseOrder(LibraryProgram $program): array
    {
    ...
    {
        $byCourse = $books->groupBy(fn (LibraryBook $book) => mb_strtolower(trim((string) $book->course)));
    ...
    protected function collectionTypeFor(LibraryBook $book): string
    {
